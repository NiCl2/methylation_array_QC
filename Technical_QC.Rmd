...
##Introduction
This is an R Markdown document for the weeekly Quality Control (QC) check from the Illumina EPIC microarray data from the `r args[1]` 
...
SampleSheet <- read.csv("/mnt/data1/Grantneilson/NCBI_QC/SampleSheet_NCBI.csv", stringsAsFactors = FALSE)
load("/mnt/data1/Grantneilson/NCBI_QC/NCBI_Mset.rdat")
load("/mnt/data1/Grantneilson/NCBI_QC/NCBI_RGset.rdat")

SampleSheet<-read.csv(file = args[2], stringsAsFactors = FALSE)
...
The phenotype file containing the sample information was loaded into R. Then the methylation data for the `r nrow(SampleSheet)` samples were loaded into a methylumiset. This contains `r nrow(SampleSheet[pheno$Sample_ID != "Meth_control",])` `r args[1]` and `r nrow(SampleSheet[pheno$Sample_ID == "Meth_control",])` fully methylated control samples.
...
# times<-data.frame(Date_Ran=c("06/12/2018","06/12/2018","21/12/2018","22/12/2018","20/03/2019","25/03/2019","20/03/2019","25/03/2019","19/07/2019","23/07/2019","02/08/2019","05/08/2019","30/10/2019","01/11/2019")
# add some study info and save
info<-data.frame(Study=rep(args[1],nrow(FMpheno)), iDAT_Location=rep(idatPath, nrow(FMpheno)))
...
lowintensitysamples<-which(M.median < 100 | U.median < 100)
```
...
```{r Control_Probe_analysis, echo=FALSE}
