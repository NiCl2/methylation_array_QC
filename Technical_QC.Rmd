...
Controls <- SampleSheet[SampleSheet$Control, "Basename"]
...
control_probes <- EpicManifest[which(EpicManifest$AddressA_ID %in% ctrlAddress), "Name"]

## Plotting control probes from fully methylated control
RGset_controls <- RGset[,which(RGset@colData@listData$Basename %in% Controls)]
par(mfrow=c(1,2))
...
r <- getRed(RGset)
g <- getGreen(RGset)
ctrlAddress <- getControlAddress(RGset)
