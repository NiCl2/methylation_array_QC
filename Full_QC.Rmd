...
params: 
  Name: Portland
  SampleSheet: Portland_SampleSheet_All.csv
  PathToProjectFolder: /mnt/data1/Array_Projects/Portland_Samples/All/
  Mset: Portland_Mset.rdat 
  RGset: Portland_RGset.rdat
  IntensityThreshold: 750
  Genotype: /mnt/data1/Array_Projects/Portland_Samples/Genetics/EPIC_SNPs_MSUmethylsamples.raw
  fam: /mnt/data1/Array_Projects/Portland_Samples/Genetics/EPIC_SNPs_MSUmethylsamples.fam
  SamplesPassed: SamplesPassedQC.csv
  SamplesFailed: SamplesFailedQC.csv
  Normalised: Portland_Blood_All_Samples_Normalised.rdat
...
As you can see from the plot above there is a group of highly unmethylated samples, whcih we suspected to be whole genome amplified, these will be excluded from the study as they will heavily influence normalisation.

```{r, echo = FALSE}

WGA_Samples<-which(M.median > 3000 | U.median < 4000)
```

```{r WGA Scatter, echo=FALSE}
WGA<-rep("TRUE", nrow(SampleSheet))
WGA[WGA_Samples] <-"FALSE"

plotfactor<-as.factor(WGA)

plot(M.median, U.median, pch = 16, xlab = "Median M intensity", ylab = "Median U intensity", main= "Whole Genome Amplfied Cluster", col=rainbow(2)[factor(plotfactor)])
abline(v = 3000, col = "red")
abline(h = 4000, col = "red")
legend("topleft", levels(factor(plotfactor)), pch = 16, col = rainbow(2))
```
```{r update WGA, include=FALSE}
SamplesFail[which(WGA=="TRUE")]<-TRUE
QCmetrics<-cbind(QCmetrics, WGA)
WGA_Fails<-c(sum(WGA=="TRUE"),sum(SamplesFail))
Stepsummary<-cbind(Stepsummary,WGA_Fails)
```
...
SampleSheet <- SampleSheet[,c("Basename","Family_ID","Twin_Status","Individual_ID","Sample_ID","Study_Name","Institute","Plate","Plate_Location","Sex","Age","Ethnicity","Organ","Tissue_Type","Cell_Type","Time_Point","DNA_IID","DNA_FID","Phenotype", "SentrixID","Position","M.median","U.median","ratio","WGA","GenoCor","maxunrelatedcors", "SmokingScore","Predicted_Tissue","Mismatch_Tissue")]
