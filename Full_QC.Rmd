...
if(file.exists(file= paste("/mnt/data1/Array_Projects", params$Mset, sep = "/"))){
  load(file = paste("/mnt/data1/Array_Projects", params$Mset, sep = "/"))
...
if(file.exists(file= paste("/mnt/data1/Array_Projects", params$RGset, sep = "/"))){
  load(file= paste("/mnt/data1/Array_Projects", params$RGset, sep = "/"))
...
**Sample Tissue:** `r print(unique(SampleSheet$Tissue_Type))` 
...
The fully methylated controls are now removed
...
# SampleSheet <- cbind(SampleSheet,PredictedSex)
# 
# #here we add the reported sex data 
# Sexplot_data <- cbind(Sexplot_data, SampleSheet$Sex)
# colnames(Sexplot_data)[colnames(Sexplot_data)=="SampleSheet$Sex"] <- "Reported Sex"
...
geno<-read.table(params$Genotype, row.names=1, h=T)
geno[,1:5]<-NULL
...
ID<-SampleSheet[match(rownames(geno),SampleSheet$DNA_IID),]

geno<-geno[match(SampleSheet$DNA_IID, rownames(geno)),]
id<-geno[-which(rownames(geno) %in% SampleSheet$DNA_IID),]
...
Of the `r nrow(SampleSheet)` samples, `r sum(is.na(SampleSheet$DNA_IID))` do not have genotype information available which overlap the DNAm array geno SNPs. 

Of the `r sum(!is.na(SampleSheet$DNA_IID))` which do, `r sum(GenoCor<0.8, na.rm=T)` have a correlation < 0.8 with their genotypes and fail the QC. The samples which are failing at this step are fairly evenly spread across the plates and chips so it doesn't appear to be chip mix up.
...
brain_IDs<-unique(SampleSheet$Individual_ID) # n = 656
singles <- table(as.character(SampleSheet$Individual_ID))
singleID <- names(singles[which(singles[] == 1)])
...
brainIDs<-unique(SampleSheet$Individual_ID) 
...
counts <- estimateCellCounts(RGset, compositeCellType ="Blood", cellTypes = c("CD8T","CD4T", "NK","Bcell","Mono","Gran"), referencePlatform = "IlluminaHumanMethylationEPIC", processMethod = "auto")
...
}else if(SampleSheet$Organ == "Brain") {
  #Making cell counts file using referance data set, this takes a while so has already been done
counts <- estimateCellCounts(RGset, compositeCellType ="DLPFC", cellTypes = c("NeuN_neg","NeuN_pos"), referencePlatform = "IlluminaHumanMethylationEPIC", processMethod = "auto")
save(counts, file="CellCounts.rdat")
...
SampleSheet<-subset(SampleSheet, select=-c(Control,PredictedSex,pFilterPass,outliers$outliers,predictedduplicates,GenoCor,Intensity,maxunrelatedcors))

betas<-betas[,match(SampleSheet$Basename, colnames(betas))]

SampleSheet <- SampleSheet[order(rownames(SampleSheet)),]
betas <- betas[,order(colnames(betas))]
print(identical(rownames(SampleSheet), colnames(betas)))
