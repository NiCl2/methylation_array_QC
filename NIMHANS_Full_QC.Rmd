...
params: 
  Name: NIMHANS
  SampleSheet: nihamsSampleSheet.csv
  PathToProjectFolder: /home/nClifton/methylation/Epic_methylation_array_QC/
  Mset: QC_nimhans/nimhans_Mset.rdat 
  RGset: QC_nimhans/nimhans_RGset.rdat
  IntensityThreshold: 750
  Genotype: /mnt/data1/IndiaWorkshop/EXTEND_genotype_QCd_59EPICSNPs.csv
  fam: 
  SamplesPassed: QC_nimhans/SamplesPassedQC.csv
  SamplesFailed: QC_nimhans/SamplesFailedQC.csv
  Normalised: QC_nimhans/nimhans_Normalised.rdat
...
SampleSheet<-read.csv(file = params$SampleSheet, stringsAsFactors = FALSE, sep = ",")
SampleSheet$Sex <- as.factor(SampleSheet$Sex)

rownames(SampleSheet) <- SampleSheet$Basename

SampleSheet$Empty <- is.na(SampleSheet$Sample_Name) 
...
idatPath<-c("/mnt/data1/NIMHANS/idats/")
...
## Coloured By Sex
plotfactor <- factor(SampleSheet$Sex, levels=unique(SampleSheet$Sex))
par(mfrow = c(1,1))
plot(M.median, U.median, pch = 16, xlab = "Median M intensity", ylab = "Median U intensity", col = rainbow(nlevels(plotfactor))[factor(plotfactor)], main="Scatter plot of Signal Intensities coloured by Sex")
par(xpd=TRUE)
legend("topright", levels(factor(plotfactor)), col = rainbow(nlevels(plotfactor)), pch = 10, cex=0.5)


##Coloured by Tissue type
plotfactor<-factor(SampleSheet$BrainRegion, levels=unique(SampleSheet$BrainRegion))
par(mfrow = c(1,1))
plot(M.median, U.median, pch = 16, xlab = "Median M intensity", ylab = "Median U intensity", col = rainbow(nlevels(plotfactor))[factor(plotfactor)], main="Scatter plot of Signal Intensities coloured by Brain Region")
par(xpd=TRUE)
legend("topright", levels(factor(plotfactor)), col = rainbow(nlevels(plotfactor)), pch = 10, cex=0.5)
...
plotfactor<-factor(SampleSheet$SentrixID, levels=unique(SampleSheet$SentrixID))
par(mfrow = c(1,1))
...
Samples with SentrixID 29 have higher M intensity
...
plot(M.median, U.median, pch = 16, xlab = "Median M intensity", ylab = "Median U intensity", col=rainbow(2)[factor(plotfactor)], main = "Intensity check")
...
# convert predicted sex to full words
PredictedSex[PredictedSex == "M"] <- "Male"
PredictedSex[PredictedSex == "F"] <- "Female"
...
2 failed sex chack due to NA value. Cluster with females
...
# if(!is.null(SampleSheet[1,"DNA_IID"])){
...
for (region in unique(SampleSheet$BrainRegion)){
  sample_names <- unique(SampleSheet$Sample_Name[SampleSheet$BrainRegion == region])
  unrelatedcors <- snpCor
  for (i in sample_names){
    samples <- which(SampleSheet$Sample_Name == i)
    unrelatedcors[samples,samples] <- NA
  }
  unrelatedcors <- unrelatedcors[SampleSheet$Basename[SampleSheet$Sample_Name %in% sample_names], SampleSheet$Basename[SampleSheet$Sample_Name %in% sample_names]]
  maxunrelatedcors <- apply(unrelatedcors, 1, max, na.rm = TRUE)
  
  hist(maxunrelatedcors, main=paste("Maximum correlation in samples from unrelated individuals:", region), xlab="Max Correlation")
}
```
...
We can see there is a correlation between predicted and reported ages in the samples `r AgeCorTest$estimate`. Generally, Horvath's clock doesn't predict as well in older and brain samples. The predicted ages are added to the QC output.
...
##Tissue Prediction
Only differentiates blood, buccal and saliva

```{r Tissue Prediction, echo =FALSE, message=FALSE, results= 'hide'}
#load function 
source("/mnt/data1/Array_Projects/Tissue_Predictor/TissuePredictorScripts/Tissue_predictor_function.R")

QCmetrics <- cbind(QCmetrics, tissue_predictor(betas)) #QCmetrics, msetEPIC
ggplot(data = QCmetrics, aes(x=BrainRegion, fill= Predicted_Tissue))+
...
# save(msetEPIC.previo, file = "QC_nimhans/mset_removedfailed.rdat")
load("QC_nimhans/mset_removedfailed.rdat")
 

#then use dasen to normalise
# msetEPIC.dasen.previo <- dasen(msetEPIC.previo)
# save(msetEPIC.dasen.previo, file = "QC_nimhans/mset_dasen.rdat")
load("QC_nimhans/mset_dasen.rdat")
```
...
setwd(params$PathToProjectFolder)
if(!is.null(SampleSheet$Organ) && SampleSheet$Organ == "Blood"){
...
SampleSheet <- SampleSheet[,c("Basename","ID","Sample_Name","Sex","Age","BrainRegion","SentrixID","Position","M.median","U.median","SmokingScore", "PredictedAge", "Predicted_Tissue")]
